...
const AdminEventItemComponent = ({ event }) => {
  ...
  return (
    <AdminEventItemContentBlock>
      <AdminEventItemBlock><h2>{eventNum}</h2></AdminEventItemBlock>
      <AdminEventItemBlock><h2>{categoryId}</h2></AdminEventItemBlock>
      <AdminEventItemBlock><h2>{userId}</h2></AdminEventItemBlock>
      <AdminEventItemBlock><h2>{eventTitle}</h2></AdminEventItemBlock>
      <AdminEventItemBlock><h2>{startEventDate}</h2></AdminEventItemBlock>
      <AdminEventItemBlock><h2>{endEventDate}</h2></AdminEventItemBlock>
      <AdminEventItemBlock><h2>{formattedDate}</h2></AdminEventItemBlock>
      <AdminEventItemBlock><h2>{formattedUpdateDate}</h2></AdminEventItemBlock>
      <AdminEventItemBlock><h2>{view}</h2></AdminEventItemBlock>
    </AdminEventItemContentBlock>
  );
};
      ...
      <AdminEventListBlock>
        ...
        {!loading && events && (
          <AdminEventItemBlock>
            {events &&
              events.map((event) => (
                <Link key={event.eventNum} to={`/admin/event/detail/${event.eventNum}`}>
